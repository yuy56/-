            ...
            <!-- 表格：展示平台属性 -->
                ...
                <el-table-column prop="prop" label="属性值列表" width="width">
                    <template slot-scope="{row}">
                        <el-tag type="success" v-for="attrValue in row.attrValueList" :key="attrValue.id" style="margin:0px 20px">
                        {{ attrValue.valueName }}
                        </el-tag>
                    </template>
                </el-table-column>
            ...
            <div v-show="!isShowTable">
                ...
                <el-table style="width:100%;margin:20px 0px" border :data="attrInfo.attrValueList">
                    ...
                    <el-table-column width="width" prop="prop" label="操作">
                        <template slot-scope="{row,$index}">
                            <!-- 气泡确认框 -->
                            <el-popconfirm :title="`确定删除${row.valueName}?`" :ref="$index" @onConfirm="deleteAttrValue($index)">
...
export default {
    ...
    methods: {
        ...
        addAttrValue(){
            //向属性值的数组里面添加元素
            //attrId:是相应属性的id，目前而言我们是添加属性的操作，还没有相应的属性的id，目前而言带给服务器的id为undefined
            //valueName：相应的属性值的名称
        ...
        toEdit(row,index){
            row.flag=true
            //获取input节点，实现自动聚焦
            //需要注意：点击span的时候，切换为input变为编辑模式，但是需要注意，对于浏览器而言，页面重绘与重排需要耗时
            //点击span的时候，重绘一个input需要耗时，所以不能一点击就马上获取到input
        ...
        //气泡确认框确定按钮的回调
        //最新版本的ElementUI---2.15.13，目前模版中的版本号2.23.x
